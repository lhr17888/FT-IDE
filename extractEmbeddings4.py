...
# count = 11633
# with open('./data/ENTITY/4/ollama embeddings.txt', 'r') as embeddingsfile:
#     for i in embeddingsfile.readlines():
#         vec1 = eval(i.strip().split('\t')[1])
#         if vec1 == None:
#             count += 1
#             with open(f'./data/ENTITY/4/similarity/{count}similarity.txt', 'w') as file:
#                 file.write('[]')
#             continue
#         list_similarity = []
#         with open('./data/ENTITY/4/ollama embeddings.txt', 'r') as file:
#             current = 11633
...
# 示例用法
directory = './data/ENTITY/4/similarity'  # 指定你的文件夹路径
read_all_txt_files(directory)

with open('./data/ENTITY/4/similarity.txt', 'w') as file:
