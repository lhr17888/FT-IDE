...
class KnowledgeGraph():
    ...
    def ordered_by_nodes_degree_and_edges(self, new_ordered_train_data, ss_id):
        ...
        for (h, r, t) in new_ordered_train_data:
            ...
            tmp_ordered_train_data.append((h, r, t, v))
        tmp_ordered_train_data.sort(key=lambda x: x[3], reverse=True)
        new_ordered_train_data = list(map(lambda x: (x[0], x[1], x[2]), tmp_ordered_train_data)) # 每层三元组按重要性排序
        assert ori_len == len(new_ordered_train_data)
        return new_ordered_train_data
    ...
    def generate_layers(self, args):
        ...
        train_data = list(map(lambda x: (x[0], x[1], x[2]), tmp_train_data)) # 按权重降序之后的三元组
        ...
        for ss_id in range(1, int(self.args.snapshot_num)):
            ...
            flag = True
            lay_id = 1
            while flag: # 实现每层三元组按重要性排序
                """ Step1: add triples with at least one old entity """
                new_entities_ = set()
                ...
                train_data = list(filter(lambda x: x[0] not in old_entities and x[2] not in old_entities, train_data)) # 下一层的待选择三元组
                """ Step3: update old graphs """
                old_entities = (old_entities | new_entities_) # 将该层中的新实体加入到实体集中
