...
class TrainBatchProcessor():
    ...
    def process_epoch(self, model, optimizer):
        ...
        # for b_id, batch in enumerate(self.data_loader):
        #     print(b_id, len(batch[0]), batch[0])
        #     print(b_id, len(batch[1]), batch[1])
        #     print(b_id, len(batch[2]), batch[2])
        #     print(b_id, len(batch[3]), batch[3])
        if self.args.record:
        ...
        for b_id, batch in enumerate(self.data_loader):
            ...
            # tensor4 = by.numpy()
            # # 保存到文件
            # np.savez(f'tensors{b_id}.npz', tensor1=tensor1, tensor2=tensor2, tensor3=tensor3, tensor4=tensor4)
            # print(len(by))
