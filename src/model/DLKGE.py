...
class DLKGE(BaseModel):
    ...
    def pre_snapshot(self):
        ...
        if self.args.using_mask_weight and self.args.snapshot:
            self.num_new_entity = self.kg.snapshots[self.args.snapshot].num_ent - self.kg.snapshots[
                self.args.snapshot - 1].num_ent
            self.entity_weight_linear = nn.Linear(self.num_new_entity, self.num_new_entity, bias=False)
            self.relation_weight_linear = nn.Linear(self.num_new_relation, self.num_new_relation, bias=False)
...
class TransE(DLKGE):
    ...
    def get_semantic_distill(self, old_ent_embeddings, new_ent_embeddings):
        loss = 0.0
        # 绝对路径
        with open(f'./data/ENTITY/{self.args.snapshot}/similarity.txt', 'r') as similarity_file:
            similar_dict = eval(similarity_file.read())
            for i in similar_dict:
                similar_list = similar_dict[i]
                for j in similar_list:
                    old = old_ent_embeddings[i-1]
                    new = new_ent_embeddings[j-1]
                    if torch.all(old.eq(0)) or torch.all(new.eq(0)):
                        continue
                    loss += self.huber_loss(old, new)
        loss /= 2
        return loss

    '''------------------------- 以上是自己写的部分 -------------------------'''

    def get_multi_layer_loss(self, lens, entity_mask, relation_mask, entity_mask_weight):
        ...
        # if self.args.snapshot == 0 or (self.args.use_two_stage and self.args.epoch < 10):
            return 0.0
        ...
        # print(11111)
        # print(old_ent_embeddings)
        # print(len(old_ent_embeddings))
        # print(new_ent_embedidngs)
        # print(len(new_ent_embedidngs))

        # loss = 0.0
        # if self.args.epoch >= 20:
        ...
        #     for j in range(i + 1, len(old_ent_embeddings)):
                # dot_product = torch.dot(new_ent_embeddings[i], new_ent_embeddings[j])
                # norm_i = torch.norm(new_ent_embeddings[i])
                # norm_j = torch.norm(new_ent_embeddings[j])
                # cos_sim = dot_product / (norm_i * norm_j)
                # if cos_sim >= 0.8:
                #     loss += self.huber_loss(new_ent_embeddings[i], new_ent_embeddings[j])

        '''------------------------- 防止某些向量对过近 -------------------------'''

        # 归一化向量
        normalized_embeddings = F.normalize(new_ent_embeddings, p=2, dim=1)

        # 计算余弦相似度矩阵
        cosine_similarity_matrix = torch.matmul(normalized_embeddings, normalized_embeddings.T)

        # 找出相似度大于等于 0.9 的索引（只保留上三角矩阵中的值，去掉对角线）
        mask = torch.triu(torch.ones_like(cosine_similarity_matrix), diagonal=1)
        high_similarity_mask = (cosine_similarity_matrix >= 0.9) & mask.bool()
        high_similarity_indices = torch.nonzero(high_similarity_mask)
        ...
        # print(torch.all(old_ent_embeddings[0].eq(0)))
        # print(torch.all(old_ent_embeddings[5816].eq(0))

        '''------------------------- 以下是自己写的部分 -------------------------'''

        # if len(lens) % self.args.batch_size and self.args.epoch > 20 and self.args.epoch <= 30:
        # if len(lens) % self.args.batch_size:
        # if len(lens) % self.args.batch_size and self.args.epoch > 20 and self.args.epoch <= 30 and self.args.snapshot < 3:
        # if len(lens) % self.args.batch_size and self.args.epoch < 20:
        # if len(lens) % self.args.batch_size and not self.args.epoch % 5:
        # loss += self.get_semantic_distill(old_ent_embeddings, new_ent_embedidngs)
        ...
        if self.args.using_relation_distill and self.args.epoch >= 20:
    ...
    def loss(self, head, relation, tail=None, label=None, entity_mask=None, relation_mask=None,
             entity_mask_weight=None):
        loss = 0.0
        ...
        if self.args.use_multi_layers and (not self.args.without_multi_layers):
            multi_layer_loss = self.get_multi_layer_loss(head, entity_mask, relation_mask, entity_mask_weight)
